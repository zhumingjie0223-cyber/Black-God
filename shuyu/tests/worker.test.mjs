...
test('GET /turn 转息与参数校验', async () => {
  const env = mockEnv();
  const r = await (await call(env, '/turn?word=0&at=60&n=3')).json();
  assert.equal(r.栖, true);
  assert.equal(r.种.id, 0);
  assert.equal((await call(env, '/turn')).status, 400);
  assert.equal((await call(env, '/turn?word=0&at=1700000000&n=1')).status, 400);
...
test('GET /incline 倾息与参数校验', async () => {
  const env = mockEnv();
  const r = await (await call(env, '/incline?word=0&at=60&n=3')).json();
  assert.equal(r.顾, true);
  assert.equal(r.种.id, 0);
  assert.equal((await call(env, '/incline')).status, 400);
  assert.equal((await call(env, '/incline?word=0&at=1700000000&n=1')).status, 400);
  assert.equal((await call(env, '/incline?word=绝不存在&at=0')).status, 400);
});

test('GET /nestle 贴息与参数校验', async () => {
  const env = mockEnv();
  const r = await (await call(env, '/nestle?word=0&at=60&n=3')).json();
  assert.equal(r.倾, true);
  assert.equal(r.种.id, 0);
  assert.equal((await call(env, '/nestle')).status, 400);
  assert.equal((await call(env, '/nestle?word=0&at=1700000000&n=1')).status, 400);
  assert.equal((await call(env, '/nestle?word=绝不存在&at=0')).status, 400);
});

test('GET /hold 含息与参数校验', async () => {
  const env = mockEnv();
  const r = await (await call(env, '/hold?word=0&at=60&n=3')).json();
  assert.equal(r.贴, true);
  assert.equal(r.种.id, 0);
  assert.equal((await call(env, '/hold')).status, 400);
  assert.equal((await call(env, '/hold?word=0&at=1700000000&n=1')).status, 400);
  assert.equal((await call(env, '/hold?word=绝不存在&at=0')).status, 400);
});

test('GET /warm 温息与参数校验', async () => {
  const env = mockEnv();
  const r = await (await call(env, '/warm?word=0&at=60&n=3')).json();
  assert.equal(r.含, true);
  assert.equal(r.种.id, 0);
  assert.equal((await call(env, '/warm')).status, 400);
  assert.equal((await call(env, '/warm?word=0&at=1700000000&n=1')).status, 400);
...
test('GET / 与 /status 带轴尺寸与新路由清单', async () => {
  ...
  assert.ok(root.endpoints.some(e => e.startsWith('/incline')));

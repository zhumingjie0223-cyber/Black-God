...
class TestSearchCompose(unittest.TestCase):
    ...
    def test_gaze_looks_onward_without_wrap(self):
        air = e.gaze(0, 0, 3)
        looked = e.turn(0, 60, 3)
        gazed = e.gaze(0, 60, 3)
        keys = ("c", "m", "s", "k", "p")
        ...
        self.assertFalse(air["转"])
        self.assertEqual(air["id"], e.turn(0, 0, 3)["id"])
        self.assertEqual(air["转处"]["id"], air["id"])
        self.assertEqual(gazed["转"], looked["转"])
        self.assertEqual(gazed["转处"]["id"], looked["id"])
        if looked["转"] and (60 // 1920) % 2 == 0:
            self.assertTrue(gazed["顾"])
            axis = e._AXIS_NAMES.index(looked["轴"])
            sizes = [e.NC, e.NM, e.NS, e.NK, e.NP]
            nxt = looked["坐标"][keys[axis]] + looked["向"]
            if 0 <= nxt < sizes[axis]:
                self.assertNotEqual(gazed["id"], looked["id"])
                self.assertEqual(gazed["坐标"][keys[axis]], nxt)
                self.assertEqual(gazed["轴"], looked["轴"])
            else:
                self.assertEqual(gazed["id"], looked["id"])
        else:
            self.assertFalse(gazed["顾"])
            self.assertEqual(gazed["id"], looked["id"])
        edge = e.decode(CAP_EXPECTED - 1)
        stay_air = e.gaze(edge["汉"], 0, 2)
        self.assertFalse(stay_air["顾"])
        stay_down = e.gaze(edge["汉"], 60, 2)
        self.assertEqual(stay_down["转处"]["id"], e.turn(edge["汉"], 60, 2)["id"])
        with self.assertRaises(ValueError):
            e.gaze(0, 1700000000, 1)
        with self.assertRaises(ValueError):
            e.gaze(0, -1, 3)
    ...
    def test_nestle_closes_without_wrap(self):
        air = e.nestle(0, 0, 3)
        leaned = e.incline(0, 60, 3)
        nestled = e.nestle(0, 60, 3)
        held = e.nestle(0, 7680, 3)
        keys = ("c", "m", "s", "k", "p")
        self.assertFalse(air["贴"])
        self.assertFalse(air["倾"])
        self.assertEqual(air["id"], e.incline(0, 0, 3)["id"])
        self.assertEqual(air["倾处"]["id"], air["id"])
        self.assertEqual(nestled["倾"], leaned["倾"])
        self.assertEqual(nestled["倾处"]["id"], leaned["id"])
        if leaned["倾"] and (60 // 7680) % 2 == 0:
            self.assertTrue(nestled["贴"])
            axis = e._AXIS_NAMES.index(leaned["轴"])
            sizes = [e.NC, e.NM, e.NS, e.NK, e.NP]
            nxt = leaned["坐标"][keys[axis]] + leaned["向"]
            if 0 <= nxt < sizes[axis]:
                self.assertNotEqual(nestled["id"], leaned["id"])
                self.assertEqual(nestled["坐标"][keys[axis]], nxt)
                self.assertEqual(nestled["轴"], leaned["轴"])
            else:
                self.assertEqual(nestled["id"], leaned["id"])
        else:
            self.assertFalse(nestled["贴"])
            self.assertEqual(nestled["id"], leaned["id"])
        self.assertEqual(held["倾"], e.incline(0, 7680, 3)["倾"])
        self.assertFalse(held["贴"])
        self.assertEqual(held["id"], e.incline(0, 7680, 3)["id"])
        edge = e.decode(CAP_EXPECTED - 1)
        stay_air = e.nestle(edge["汉"], 0, 2)
        self.assertFalse(stay_air["贴"])
        stay_down = e.nestle(edge["汉"], 60, 2)
        self.assertEqual(stay_down["倾处"]["id"], e.incline(edge["汉"], 60, 2)["id"])
        with self.assertRaises(ValueError):
    ...
    def test_warm_heats_without_wrap(self):
        ...
        with self.assertRaises(ValueError):
            e.warm(0, -1, 3)
    def test_rouse_wakes_without_wrap(self):
        air = e.rouse(0, 0, 3)
        warmed = e.warm(0, 60, 3)
        roused = e.rouse(0, 60, 3)
        closed = e.rouse(0, 61440, 3)
        keys = ("c", "m", "s", "k", "p")
        sizes = [1040, 180, 80, 64, 8]
        self.assertEqual(air["id"], e.warm(0, 0, 3)["id"])
        self.assertFalse(air["醒"])
        self.assertEqual(roused["温"], warmed["温"])
        self.assertEqual(roused["温处"]["id"], warmed["id"])
        if warmed["温"] and (60 // 61440) % 2 == 0:
            self.assertTrue(roused["醒"])
            axis = e._AXIS_NAMES.index(warmed["轴"])
            nxt = warmed["坐标"][keys[axis]] + warmed["向"]
            if 0 <= nxt < sizes[axis]:
                self.assertNotEqual(roused["id"], warmed["id"])
                self.assertEqual(roused["坐标"][keys[axis]], nxt)
                self.assertEqual(roused["轴"], warmed["轴"])
            else:
                self.assertEqual(roused["id"], warmed["id"])
        else:
            self.assertFalse(roused["醒"])
            self.assertEqual(roused["id"], warmed["id"])
        self.assertEqual(closed["温"], e.warm(0, 61440, 3)["温"])
        self.assertFalse(closed["醒"])
        self.assertEqual(closed["id"], e.warm(0, 61440, 3)["id"])
        edge = e.decode(CAP_EXPECTED - 1)
        stay_air = e.rouse(edge["汉"], 0, 2)
        self.assertFalse(stay_air["醒"])
        stay_down = e.rouse(edge["汉"], 60, 2)
        self.assertEqual(stay_down["温处"]["id"], e.warm(edge["汉"], 60, 2)["id"])
        with self.assertRaises(ValueError):
            e.rouse(0, 1700000000, 1)
        with self.assertRaises(ValueError):
            e.rouse(0, -1, 3)
...
class TestCLI(unittest.TestCase):
    ...
    def test_search_compose_coin(self):
        ...
        self.assertEqual(self._json_tail(out)["侧"], 1)
        rc, out = self.run_cli("--gaze", "0", "--at", "60", "--n", "3")
        self.assertEqual(rc, 0)
        ...
        rc, out = self.run_cli("--nestle", "0", "--at", "60", "--n", "3")
        self.assertEqual(rc, 0)
        self.assertEqual(self._json_tail(out)["种"]["id"], 0)
        ...
        rc, out = self.run_cli("--rouse", "0", "--at", "60", "--n", "3")
        self.assertEqual(rc, 0)
        self.assertEqual(self._json_tail(out)["种"]["id"], 0)
        self.assertIn("醒", self._json_tail(out))

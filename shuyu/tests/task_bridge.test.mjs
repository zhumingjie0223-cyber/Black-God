...
test('邻近、时间与一息编译为有界工具，规划标题用时区',()=>{
  ...
  assert.equal(program.actions[6].arguments.operation,'落息');
  assert.equal(program.actions[7].arguments.operation,'起息');
  const nest=compileTask('行：栖息("1700000000")');
  ...
  const lean=compileTask('行：倾息("1700000000")');
  assert.equal(lean.actions[0].arguments.operation,'倾息');
  const nestle=compileTask('行：贴息("1700000000")');
  assert.equal(nestle.actions[0].arguments.operation,'贴息');
  const hold=compileTask('行：含息("1700000000")');
  assert.equal(hold.actions[0].arguments.operation,'含息');
  const warm=compileTask('行：温息("1700000000")');
  assert.equal(warm.actions[0].arguments.operation,'温息');
  const rouse=compileTask('行：醒息("1700000000")');
  assert.equal(rouse.actions[0].arguments.operation,'醒息');

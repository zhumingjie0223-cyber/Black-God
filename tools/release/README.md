...
Alpine 3.22.5 security1 contains 16 installed binary packages from 10 source origins. Obtain each origin's `main/<origin>` directory from the exact aports commit in its installed APK database. Keep all recipe files. Fetch the distfiles named by each APKBUILD and verify every SHA-512 checksum without executing the APKBUILD on the host. The alpine-base recipe generates release metadata and has no external source archive. Record packages, origin commits and checked files in `manifest.json`; incomplete or mismatched collections are rejected by the packager.

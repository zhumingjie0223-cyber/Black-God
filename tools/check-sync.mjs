...
// 当前消费方是iOS资源，校验失败即阻断。
if (args.length === 0 && existsSync(path.join(HERE, 'ios-app/ShuyuRuntime'))) {
  execFileSync('python3', [path.join(HERE, 'tools/shuyu/bundle.py'), '--check'], {stdio: 'inherit'});
  process.exit(0);
}
...
if (!self) {
  console.error("当前目录不存在可对比的消费引擎，不能进行跨副本同步校验。");
  process.exit(1);
}

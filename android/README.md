...
**⚠ 上架前需处理的唯一半成品**：设置 →「大脑 · 模型」选择器
(`index.html:1457-1467`,`MODELS`+`pickModel`)——点选后只写 `localStorage` + 弹 toast,
但 `Nexus.talk` 请求体不含 model 字段、后端 `/talk` 也不读它,选了跟没选一样(静默无效,
会让用户误以为切换了模型)。**待权哥定夺**:① 接进 `/talk` 让后端按选择切 tier;或 ② 删掉
该菜单项免误导。实际算力路由现由后端 `pickTier(text,caps)` + 网关配置决定,与此菜单无关。

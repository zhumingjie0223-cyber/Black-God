...
**现状(2026-07 核实,如实描述,不许骗)**:主线 UI 已采用「深海·潮光」色系
——`web/nexus-do/index.html` 是自包含单文件(构建时整页注入 Worker),CSS 令牌**内联在页内**
(`:root` / `:root[data-theme="dark"|"light"]`,潮光青 `#4FC4D9` 为主强调,深浅双主题)。
设计愿景规范见 `docs/design/DESIGN_SYSTEM_V3.md`(色系/字体五族/圆角/动效/无障碍完整规范)
与竞品分析 `docs/design/UI_REDESIGN_ANALYSIS.md`——这是**目标态蓝图**,非当前逐字实现。

- 改 UI 颜色/圆角/动效,先读 `DESIGN_SYSTEM_V3.md` 对齐色系与命名,别另起炉灶配色;
  动效只用 transform/opacity;新色值优先沿用页内既有 `--` 令牌,不随手硬编码。
...
> **未竟事项(未来单独任务,别当已完成)**:把页内内联令牌抽成真正的单一事实源
> `web/design/tokens.css` 并接进 `build.mjs` 构建注入(设计文档 §8 描述的落地方式)——
> 当前**尚未接入**,页面各自演化过颜色(如登录门底色已与文档取值漂移),
> 要做需真机/无头浏览器逐页核对上线观感不回退。在此之前,`DESIGN_SYSTEM_V3.md` 是参考蓝图,
> 不是可直接 `<link>` 的运行时文件。

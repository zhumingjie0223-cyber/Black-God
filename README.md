...
枢语执行方言v2示例：`行：计算("12*3") → "36"`。程序最多8行，可映射到已开放的计算、枢语、检索、规划、核对和执行工具，先完整编译和检查工具可用性，之后逐项执行；失败或预期不符停止。旧do:写法作为别名保留。词汇编号与词根未改动，约76.7亿是组合地址空间，不是已掌握的智能能力数量。五维类比造词按轴取模，不扩大空间。

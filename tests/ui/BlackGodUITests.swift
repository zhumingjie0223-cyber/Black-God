...
final class BlackGodUITests: XCTestCase {
    ...
    func testChatComposerChipsUseJadePrompts() {
        ...
        XCTAssertTrue(["在场", "在听", "在做", "接得上", "该练"].contains(app.staticTexts["chat.mood"].label))
        let field = app.descendants(matching: .any)["chat.input"]
        XCTAssertTrue(field.waitForExistence(timeout: 4))
        field.tap()
        XCTAssertEqual(app.staticTexts["chat.mood"].label, "在场")
        app.buttons["chat.chip.plan"].tap()
        XCTAssertEqual(app.staticTexts["chat.mood"].label, "在听")
        expectation(for: NSPredicate(format: "label == %@", "等你写完"), evaluatedWith: app.staticTexts["chat.nextWork"])
        waitForExpectations(timeout: 3)
        app.descendants(matching: .any)["chat.title"].tap()
        expectation(for: NSPredicate(format: "label == %@", "你写到这儿了"), evaluatedWith: app.staticTexts["chat.nextWork"])
        waitForExpectations(timeout: 3)
        field.tap()
        ...
        expectation(for: NSPredicate(format: "label == %@", "你收回去了"), evaluatedWith: app.staticTexts["chat.nextWork"])

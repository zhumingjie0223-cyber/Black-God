...
final class NexusShuyuTests: XCTestCase {
    ...
    func testNearClockAndRankedSearchStayBounded() async throws {
        ...
        let gaze = try JSONSerialization.jsonObject(with: Data(try NexusShuyuEngine.shared.invoke("顾息", input: #"[0,"60",3]"#).utf8)) as? [String: Any]
        XCTAssertEqual((gaze?["转"] as? NSNumber)?.boolValue, true)
        ...
        let nestle = try JSONSerialization.jsonObject(with: Data(try NexusShuyuEngine.shared.invoke("贴息", input: #"[0,"60",3]"#).utf8)) as? [String: Any]
        XCTAssertEqual((nestle?["倾"] as? NSNumber)?.boolValue, true)
        XCTAssertEqual((nestle?["种"] as? [String: Any])?["id"] as? NSNumber, 0)
        let nestleProgram = try NexusShuyuEngine.shared.compile("行：贴息(\"1700000000\")")
        XCTAssertEqual(nestleProgram.actions[0].arguments["operation"], "贴息")
        let hold = try JSONSerialization.jsonObject(with: Data(try NexusShuyuEngine.shared.invoke("含息", input: #"[0,"60",3]"#).utf8)) as? [String: Any]
        XCTAssertEqual((hold?["贴"] as? NSNumber)?.boolValue, true)

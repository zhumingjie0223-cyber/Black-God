...
> ⚠️ **实现状态(2026-07-17 核实,不许骗)**:本文是**设计愿景蓝图/目标态**,不是当前逐字实现。
> 现状——主线 UI 已采用「深海·潮光」色系,但 CSS 令牌是**内联在 `web/nexus-do/index.html` 页内**
> (`:root`/`data-theme`),下文 §8 描述的单一事实源 `web/design/tokens.css` + 构建注入**尚未接入**;
> 且页面各自演化过颜色(如登录门底色已与本文取值漂移)。把本文当**改 UI 时对齐色系/命名的参考**,
> 别当可直接 `<link>` 的运行时文件。真正抽令牌接构建是未来单独任务(需逐页核对上线观感不回退)。

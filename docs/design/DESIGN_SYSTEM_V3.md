...
> ⚠️ **实现状态(2026-07-17 无头浏览器逐屏核实,不许骗)**:本文是一份**未被采用的备选设计方向**,
> **不是当前实现**。经核实,主线 `web/nexus-do/index.html` 实际用的是「**石墨暗流 · 玉绿**」色系
> (主强调玉绿 `#3DDC84`/`#2FB96B`,令牌内联在页内 `:root`/`data-theme`),**并非**本文的「深海·潮光」青。
> 本文的潮光青 #4FC4D9、`web/design/tokens.css`、§8 构建注入等**均未落地**。
> **权哥 2026-07-17 拍板:保留石墨绿现状**;是否改成潮光青属未来单独立项。
> 在此之前,本文仅作历史设计探索留存,不代表线上观感,别当"该对齐"的目标。

> (以下为原始愿景规范,措辞保留"必须/单一事实源"等目标态口吻,按上方状态理解)
> 所有 Web 页面（index / nexus-do / studio / voice）与 iOS Theme 目标走本 token 体系；
> 页面私设色值/圆角/动效 = 违规。目标单一事实源：`web/design/tokens.css`。

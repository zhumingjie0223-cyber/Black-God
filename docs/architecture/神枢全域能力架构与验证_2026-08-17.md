...
**审计日期：**2026-08-17  
**神枢仓库基线：**`Black-God/main`，当前 GitHub 已推送版本 `1e87c54`
**工程研究边界：**本报告只固化神枢自身实现、枢语协议与运行证据；所有外部开源研究均为只读背景材料，不构成神枢产品、命名、运行依赖或代码来源。
**目的：**本文件是**神枢全域能力架构与验证报告**。它只以神枢在 `Black-God` 仓库中的真实调用链、枢语协议、Cloudflare Durable Object、Container、iOS 客户端和运行证据作为验收对象；外部研究仅作为内部审计背景。

> **核心结论：**神枢是以枢语语义、Cloudflare 中枢、Container 浏览器与 Linux 执行面、Durable Object 账本、Web 工作台和 iOS 受控桥接组成的独立产品。升级目标是让这些神枢自身模块收敛为一条可恢复、可审计、可实测的统一运行链，而不是固化、复制或依赖任何外部项目。

## 1. 能力地图总览

| 神枢能力层 | 神枢验收原则 | 神枢实际运行能力 | 当前判定 | 神枢落地目标 |
|---|---|---|---|---|
| Agent 回合状态机 | 规范 history 与展示消息分离；工具结果按源顺序回灌；回合可恢复 | `ShenshuCore.runAgentLoop()` 最多 5 轮，解析 `⟨工具:...｜...⟩` 文本标记；`AgentStateMachineDO` 已有 plan/approval/lease/run/audit | **部分实现，尚未统一** | 让所有 Agent 工具回合使用同一个 DO run ledger、typed tool call、持久化 history 和可恢复状态；弃用将工具调用仅编码在文本中的核心依赖 |
| 工具并发与回灌完整性 | 有界十路并发、source-index 排序、工具调用与结果成对持久化 | 神枢每轮最多截取 2 个 call，按 `for...of` 串行执行；工具日志为内存数组后回给模型 | **明显缺口** | 引入有界并发执行器、稳定 `tool_call_id`、原顺序重组、已完成配对不可丢失、取消时可恢复 |
| 流式 Provider 协议 | OpenAI Chat/Responses、Anthropic、Gemini 的统一协议适配；SSE 处理 usage、reasoning、tool JSON delta | `brainProvider/buildBrainReq/parseBrainText` 支持 OpenAI/Anthropic/Gemini 的非流式单轮文本请求 | **明显缺口** | 新建 Provider Adapter 层，支持流式事件、Responses API、typed function call、reasoning 回放、token/finish 原因和 provider-specific ID 规范化 |
| 工具输入预检 | 参数归一化、required field 校验、工具专用约束和循环检测 | 神枢模型提示中规定 key=value/confirm；`parseToolCalls` 接收文本参数；危险设备操作可返回 `need_confirm` | **部分实现，强度不足** | 用 capability schema 驱动 JSON 参数验证、枚举/长度/范围/目标策略、预检报告、每次副作用的 idempotency key 与 owner confirmation |
...
| 上下文压缩 | 以真实 message id 锚定摘要；确保 Responses tool pair 不被压缩拆开 | 神枢的 `gw` 候选池与 `scratchCandidates` 上限 20；无 message-id anchored compaction | **缺口** | 实现会话级 token 预算器；压缩时保持 assistant tool call 与 tool result 原子对；将摘要与锚点写入 DB |
...
| MCP 与 OAuth | MCP registry、CLI、OAuth PKCE、Keychain 与受限执行桥接 | 神枢当前未发现 MCP server registry、JSON-RPC transport、OAuth callback/token vault 或 worker-to-tool 安全桥 | **未实现** | 建立 MCP registry、HTTP/stdio transport boundary、OAuth PKCE、secret reference token vault、按 tool 风险分级和审计；不把 refresh token 放入普通 KV/聊天记录 |
...
| 网络能力 | 受控 guest 使用宿主网络；普通 App 不承诺物理网卡控制 | 神枢 Container 能联网；iOS 无 Network Extension、VPN、DNS Proxy 或本机网络权限执行层 | **服务端网络可用；手机网络面未实现** | 建立“云端浏览器/Container + iOS 原生受控网络 + 局域网/主人节点”三平面；真实网卡控制不作为普通 App 承诺，Network Extension 只在获 entitlement 后启用 |
...
| 构建供应链与 rootfs | iSH/PRoot 子模块固定版本；rootfs 脚本必须进行 hash 校验 | Container 由 Cloudflare 建置；iOS 项目较薄；移动 Linux 无 rootfs 构建链 | **缺口** | 生成 SBOM、锁定 Container image digest、对任何 Alpine/rootfs/PRoot 输入验证 SHA256 与签名、CI 中执行 reproducibility check |
| 可观测性与测试 | Provider/tool/compaction/OAuth 需要协议级测试，原生帧与 handler 需要专门回归 | 神枢已有 94 项 Node 回归、Workers observability、终端实时输出；生产未完成 owner-path 全链实测 | **部分实现，实测不足** | 建立分层测试矩阵：unit、protocol frame、container integration、Worker/DO integration、iOS simulator/device、production owner-path；每项保存匿名化运行证据 |
...
现有 `runAgentLoop()` 使用模型输出中的文本工具标记，最多循环 5 次、每轮最多串行处理 2 个工具。它已有失败分类、重规划提示、WebSocket 事件和经验记录，但尚未具备规范化 provider history、稳定 tool ID、并发 source-order 回灌、成对事务提交或崩溃恢复。因此该函数是后续 Agent 内核迁移的入口，不是最终形态。
...
普通 iPhone App 无法直接接管 Wi-Fi、蜂窝数据或获得物理网卡驱动。神枢现有 Container 使用云端网络。真正可行的产品设计是：**Cloudflare/Container 作为云端执行平面，iOS App 作为经系统授权的原生能力平面，主人可控的局域网或长期节点作为真实网络与长期任务平面，枢语作为贯穿四者的语义/审计坐标。**
...
后续每一项改动都必须在本文件对应行追加：神枢文件与函数、枢语协议、测试入口、真实生产证据、失败或阻塞原因。未接线模块、未授权 owner-path 和未获 Apple entitlement 的能力不得写成“已实现”。
...
`nexus_tool_preflight.mjs` 已接入 `AgentStateMachineDO.handlePlan()`、`ShenshuCore.agentPlan()` 与 `ShenshuCore.invokeCapability()` 三道真实入口。它在计划和执行两端统一校验能力与字段，处理设备动作/HealthKit 枚举、路径穿越、长度、URL scheme、通知/剪贴板等副作用提示，并保留 `file_edit.new_string === ''` 的明确删除语义。无效参数不再进入 plan、approval、lease 或 handler。

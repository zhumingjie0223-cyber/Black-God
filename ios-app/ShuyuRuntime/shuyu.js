// 自动生成；修改shuyu权威源后运行 tools/shuyu/bundle.py
(function(){
"use strict";
/**
...
const CAPACITY = NC*NM*NS*NK*NP;          // 7,667,712,000（核1040×映180×态80×标64×相8，v4 追加 32 族后）
const AXES = Object.freeze({ 核:NC, 映:NM, 态:NS, 标:NK, 相:NP });
...
function compactPose(word, meta){
  return {
...
// ══════ 摇息：回息后按分钟在弹回词与末步之间摇摆；不环绕、不改编号空间 ══════
function sway(seed, at, n){
  ...
  const lean = last;
  const swaying = rest.id !== last.id;
  const pose = (swaying && side === 1) ? lean : restPose;
  return {
    息: rest.息,
    种: rest.种,
    步: rest.步,
    ...
    摇: swaying,
    侧: swaying ? side : 0,
    摆: swaying ? (side === 1 ? restPose : lean) : restPose,
...
// ══════ 转息：栖息后再朝侧邻转头；未栖不转；无侧邻则停在栖处 ══════
function turn(seed, at, n){
  const nest = perch(seed, at, n);
  const last = nest.迹[nest.迹.length - 1];
  const t = Number(at);
  const sit = compactPose(nest, { 息: last.息, 时: last.时, 分: last.分, 轴: nest.轴, 向: nest.向, 动: nest.栖 });
  const neighbors = near(nest.id, 16).filter(word => word.轴 !== nest.轴);
  const turning = nest.栖 === true && neighbors.length > 0;
  const pick = turning ? neighbors[Math.floor(t / 960) % neighbors.length] : null;
  const keys = ['c', 'm', 's', 'k', 'p'];
  let pose = sit;
  if(pick){
    const axis = AXIS_NAMES.indexOf(pick.轴);
    const direction = pick.坐标[keys[axis]] > nest.坐标[keys[axis]] ? 1 : -1;
    pose = compactPose(pick, { 息: last.息, 时: last.时, 分: last.分, 轴: pick.轴, 向: direction, 动: true });
  }
  return {
    息: nest.息,
...
// ══════ 倾息：望出去后再沿望轴倾近一格；未顾不倾；六十四分钟窗才倾；越界停在望处 ══════
function incline(seed, at, n){
  const gazed = gaze(seed, at, n);
  const last = gazed.迹[gazed.迹.length - 1];
  const t = Number(at);
  const look = compactPose(gazed, { 息: last.息, 时: last.时, 分: last.分, 轴: gazed.轴, 向: gazed.向, 动: gazed.顾 });
  const keys = ['c', 'm', 's', 'k', 'p'];
  let pose = look;
  let inclining = false;
  if(gazed.顾 === true && Math.floor(t / 3840) % 2 === 0){
    inclining = true;
    const sizes = [NC, NM, NS, NK, NP];
    const axis = AXIS_NAMES.indexOf(gazed.轴);
    const coord = keys.map(key => gazed.坐标[key]);
    const next = coord[axis] + gazed.向;
    if(next >= 0 && next < sizes[axis]){
      coord[axis] = next;
      pose = compactPose(decode(idOf(...coord)), { 息: last.息, 时: last.时, 分: last.分, 轴: gazed.轴, 向: gazed.向, 动: true });
    }
  }
  return {
    息: gazed.息,
...
// ══════ 贴息：倾近后再贴住一格；未倾不贴；一百二十八分钟窗才贴；越界停在倾处 ══════
function nestle(seed, at, n){
  const leaned = incline(seed, at, n);
  const last = leaned.迹[leaned.迹.length - 1];
  const t = Number(at);
  const rest = compactPose(leaned, { 息: last.息, 时: last.时, 分: last.分, 轴: leaned.轴, 向: leaned.向, 动: leaned.倾 });
  const keys = ['c', 'm', 's', 'k', 'p'];
  let pose = rest;
  let nestling = false;
  if(leaned.倾 === true && Math.floor(t / 7680) % 2 === 0){
    nestling = true;
    const sizes = [NC, NM, NS, NK, NP];
    const axis = AXIS_NAMES.indexOf(leaned.轴);
    const coord = keys.map(key => leaned.坐标[key]);
    const next = coord[axis] + leaned.向;
    if(next >= 0 && next < sizes[axis]){
      coord[axis] = next;
      pose = compactPose(decode(idOf(...coord)), { 息: last.息, 时: last.时, 分: last.分, 轴: leaned.轴, 向: leaned.向, 动: true });
    }
  }
  return {
    息: leaned.息,
...
// ══════ 温息：含住后再温住一格；未含不温；五百一十二分钟窗才温；越界停在含处 ══════
function warm(seed, at, n){
  const held = hold(seed, at, n);
  const last = held.迹[held.迹.length - 1];
  const t = Number(at);
  const rest = compactPose(held, { 息: last.息, 时: last.时, 分: last.分, 轴: held.轴, 向: held.向, 动: held.含 });
  const keys = ['c', 'm', 's', 'k', 'p'];
  let pose = rest;
  let warming = false;
  if(held.含 === true && Math.floor(t / 30720) % 2 === 0){
    warming = true;
    const sizes = [NC, NM, NS, NK, NP];
    const axis = AXIS_NAMES.indexOf(held.轴);
    const coord = keys.map(key => held.坐标[key]);
    const next = coord[axis] + held.向;
    if(next >= 0 && next < sizes[axis]){
      coord[axis] = next;
      pose = compactPose(decode(idOf(...coord)), { 息: last.息, 时: last.时, 分: last.分, 轴: held.轴, 向: held.向, 动: true });
    }
  }
  return {
    息: held.息,
...
function loadCapabilities(data){
...
// ══════ 按状态自主造词：心情/情绪决定核心层 ══════
function coinFromState(soul, seed) {

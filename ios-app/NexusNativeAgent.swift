...
extension NexusToolRegistry {
    ...
    var nativeDefinitions: [NexusToolDefinition] {
        let definitions: [(String, String, [String: String], [String])] = [
            ...
            ("shuyu", "查询、编解码、类比、邻近、一息、余息、回息与检验Black God自己的枢语", ["operation": "容量/解码/拉丁编号/汉译编号/检索/造词/组合/类比/邻近/一息/余息/回息/编译/规划/往返/质数", "input": "操作输入字符串；容量可为空；类比为JSON数组；邻近为词或JSON数组；一息为Unix秒或JSON数组[词,秒]；余息与回息为Unix秒或JSON数组[词,秒,步数]"], ["operation"]),

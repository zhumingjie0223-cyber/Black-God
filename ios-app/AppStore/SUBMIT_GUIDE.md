...
构建14接续构建13，保留 uname 内存边界修复。此次修复整理失效后端流程、界面滚动定位及授权测试时序，并仅在 DEBUG 自动化进程中暂停装饰动效。Release 保留原动效、授权校验及模型与工具逻辑。

2026-09-24 23:30 实查：构建13已上传处理为 VALID，但1.3.0仍关联构建12、未送审。本次使用新构建号14，不重复上传13。完整测试、发布与审核状态另见本次交付记录，不把待执行项记为完成。

...
## 2026-09-24 — 1.3.0 真大更：打开就在场

营销号 **1.3.0**，构建 **11**。不再用小补丁号和微心情叠刀。

- 打开第一屏就能感到活：玉绿呼吸点还在跳，在场条用 transform/opacity 呼吸；有上次对话时心情是「接得上」。
- 心情收成五种人能读懂的：在场、在听、在做、接得上、该练。不再显示侧耳、偎着、挨着等微状态名词。
- 商店「此版本新功能」与 6.7 寸截图按这次真实界面重写，现有 Logo 直接放上去。
- 不绑人物角色。危险确认闸未拆。不部署服务器。

## 2026-09-17 — 第2轮前沿升级：检索排序 + 邻近词 + 可检查步骤

对照可检索、可邻近、可再生成的操作面，把当前 1.2.0 做成更好用的 App。不改词汇容量与旧编号，不重复第1轮类比造词/方言v2/芯片计划条。

### 枢语
- 语义检索按相关度排序：精确拉丁/汉 > 义段整词 > 前缀/包含，轴序与下标作并列打破；命中集合不变。
- 新增五维邻近词 `near`：只取 L1=1 且不环绕的邻居；JS/Python 同构；Worker `GET /near`；iOS 操作「邻近」。

### 任务与工具
- 执行方言增加邻近/时间（`near`/`clock`），规划标题可读时区。
- `shuyu_execute` 允许只读 `clock`；每步结果带 `s1…` 编号，便于对照失败停在哪一步。

### 操作、质感、性能
- 对话可复制、可重新生成上一则；助手气泡显示玉绿工具痕迹。
- 进入对话页时预热枢语引擎，避免第一次查询再解析脚本。

### 测试
- 枢语检索排序、邻近、方言时间；iOS 步骤编号、重新生成；`tools/shuyu/bundle.py` 再生成消费副本。

## 2026-09-17 — 第1轮前沿升级：类比造词 + 任务方言 v2 + 对话计划条

对照可见计划、快捷操作与可检查工具链，把当前 1.2.0 做成更好用的 App。不改词汇容量与旧编号。

### 枢语
- 新增五维类比造词 `analogy`：A:B :: C:? 按轴取模，JS/Python 同构；Worker `GET /analogy`；iOS 操作「类比」。
- 有界质数原语改为线性筛，结果不变。

### 任务与工具
- 执行方言升到 v2：最多 8 步，每步带 `s1…` 编号；新增规划/核对/检索。
- iOS 增加只读工具 `plan`、`verify`，纳入 `shuyu_execute` 白名单。
- 对话规划步数上限 4→8；聊天页显示当前计划条。

### 操作与质感
- 对话输入增加计算/规划/枢语快捷芯片与空状态示例；沿用页内玉绿令牌。
- 状态点与操作直播边框改用玉绿。

### 测试
- 枢语 JS/Python/Worker 与 iOS 枢语单测覆盖类比、八步、规划核对；`tools/shuyu/bundle.py` 再生成消费副本。

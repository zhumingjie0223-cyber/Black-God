...
## 2026-09-23 — 每天学习，并为每次更新做准备

神枢一天最多留一条规矩，只来自用户纠正或工具失败；同一天有新的纠正就换成最新一条。记下的内容写成「更新预备」，之后每次对话先读。没有预备，就不能把这次更新说成已经准备好。不调用额外模型，不启用知识，不改权限。休眠时不学习。用户可在「我的 → 每天学习」撤下。获准发送给模型的内容里增加了这些预备，连接页和隐私说明已写明。

...
void ft_PyObject_GetItemData(struct cpu_state *cpu, struct tlb *tlb) {
(void)tlb;(void)cpu;
    PB_LDR(cpu->regs[2], (cpu->regs[0] + 8));
    PB_LDR(cpu->regs[1], (cpu->regs[2] + 168));
    if (!((cpu->regs[1] >> 23) & 1)) goto L_e1f80;
...
L_e1f80:
    ...
    cpu->regs[30] = PB_BASE + 0xe1fa4ULL; PB_CALL(1, cpu, tlb, PB_BASE + 0x11f220ULL);
    cpu->regs[0] = 0ULL;

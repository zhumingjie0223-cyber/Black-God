/* AUTO-GENERATED prebuilt-gadget spec_fn for guest `_PyToken_OneChar` — DO NOT EDIT.
 * Source: /tmp/pyroot/fs/usr/lib/debug/usr/lib/libpython3.12.so.1.0.debug  addr 0xef9ab4e0
 * Produced by tools/prebuilt_gadget_gen/gen.sh (guest asm -> equivalent C).
 * Compiled only with -Doffload_test_prebuilt=true. */
#include <stdint.h>
#include "emu/arch/arm64/cpu.h"
#include "emu/tlb.h"
#include "kernel/native_offload.h"  /* prebuilt_call for bl/blr sites */

static uint64_t ror64(uint64_t v, unsigned r) { return (v >> r) | (v << (64 - r)); }
static uint64_t g_fa, g_fb;
static double fr[32];   /* float/double register file (s/d regs) */
/* FCMP: encode float ordering into g_fa/g_fb so the integer FLAG_* macros
 * that follow a fcmp read the right branch. sign(lhs-rhs) → (1,0)/(0,1)/(0,0);
 * NaN (unordered) → (0,1): gt/ge false, matching AArch64 fcmp semantics. */
#define FCMP(x,y) do { double _dx=(x), _dy=(y); \
    if (_dx < _dy) { g_fa=0; g_fb=1; } \
    else if (_dx > _dy) { g_fa=1; g_fb=0; } \
    else if (_dx == _dy) { g_fa=0; g_fb=0; } \
    else { g_fa=0; g_fb=1; } } while(0)   /* NaN */
#define FLAG_CMP(x,y) do { g_fa=(x); g_fb=(y); } while(0)
#define FLAG_EQ (g_fa == g_fb)
#define FLAG_NE (g_fa != g_fb)
#define FLAG_GT ((int64_t)g_fa >  (int64_t)g_fb)   /* signed */
#define FLAG_LT ((int64_t)g_fa <  (int64_t)g_fb)
#define FLAG_GE ((int64_t)g_fa >= (int64_t)g_fb)
#define FLAG_LE ((int64_t)g_fa <= (int64_t)g_fb)
#define FLAG_HI (g_fa >  g_fb)                     /* unsigned */
#define FLAG_LO (g_fa <  g_fb)
#define FLAG_HS (g_fa >= g_fb)
#define FLAG_LS (g_fa <= g_fb)
#define SP (cpu->sp)                               /* stack pointer */
/* PB_BASE: library load base = runtime addr (0xef9ab4e0) - file offset (0x00000000000874e0).
 * adrp/adr targets (file-absolute in the disassembly) resolve to
 * PB_BASE + target at runtime. No-ASLR makes this constant. */
extern uint64_t g_ft_base;
#define PB_BASE g_ft_base
/* Memory ops go through the guest TLB (fork/CoW safe). 64/32-bit + byte. */
#define PB_LDR(dst, addr)  do { uint64_t _v=0; tlb_read (tlb,(addr),&_v,8); (dst)=_v; } while(0)
#define PB_STR(addr, val)  do { uint64_t _v=(val); tlb_write(tlb,(addr),&_v,8); } while(0)
#define PB_LDRW(dst, addr) do { uint32_t _v=0; tlb_read (tlb,(addr),&_v,4); (dst)=_v; } while(0)
#define PB_STRW(addr, val) do { uint32_t _v=(uint32_t)(val); tlb_write(tlb,(addr),&_v,4); } while(0)
#define PB_LDRB(dst, addr) do { uint8_t  _b=0; tlb_read (tlb,(addr),&_b,1); (dst)=_b; } while(0)
#define PB_STRB(addr, val) do { uint8_t  _b=(uint8_t)(val); tlb_write(tlb,(addr),&_b,1); } while(0)
#define PB_LDRH(dst, addr) do { uint16_t _h=0; tlb_read (tlb,(addr),&_h,2); (dst)=_h; } while(0)
#define PB_STRH(addr, val) do { uint16_t _h=(uint16_t)(val); tlb_write(tlb,(addr),&_h,2); } while(0)
/* Inline-cache call: if the callee (guest addr `tgt`) has a translated
 * spec_fn, call it directly (stays in host code, no interpreter round-
 * trip); otherwise fall back to prebuilt_call (nested dispatch). Each
 * call site has its own static IC slot keyed by the last target. */
#define PB_CALL(id, cpu, tlb, tgt) do {                       \
    static addr_t _ic_tgt##id = 0; static prebuilt_fn _ic_fn##id = 0; \
    addr_t _t = (tgt);                                        \
    if (_t == _ic_tgt##id && _ic_fn##id) { _ic_fn##id(cpu, tlb); } \
    else { prebuilt_fn _f = native_offload_prebuilt_lookup(_t);\
           if (_f) { _ic_tgt##id = _t; _ic_fn##id = _f; _f(cpu, tlb); } \
           else { prebuilt_call(cpu, tlb, _t); } }            \
  } while(0)

void ft_PyNumber_Xor(struct cpu_state *cpu, struct tlb *tlb) {
(void)tlb;(void)cpu;
    cpu->regs[3] = PB_BASE + 0x294000ULL;
    cpu->regs[2] = 112ULL;
    cpu->regs[3] = cpu->regs[3] + 176ULL;
    { PB_CALL(1, cpu, tlb, PB_BASE + 0x12cb24ULL); return; };
    /* nop */
    /* nop */
    cpu->regs[2] = cpu->regs[1];
    PB_LDR(cpu->regs[1], (cpu->regs[0] + 16));
    if ((cpu->regs[1])==0) goto L_1fbdf4;
    PB_LDR(cpu->regs[0], (cpu->regs[0] + 40));
    { PB_CALL(2, cpu, tlb, PB_BASE + 0x162fa0ULL); return; };
L_1fbdf4:
    cpu->regs[0] = PB_BASE + 0x44f000ULL;
    PB_LDR(cpu->regs[0], (cpu->regs[0] + 3336));
    { PB_CALL(3, cpu, tlb, PB_BASE + 0x162fa0ULL); return; };
    cpu->regs[2] = cpu->regs[1];
    PB_LDR(cpu->regs[1], (cpu->regs[0] + 8));
    PB_LDR(cpu->regs[1], (cpu->regs[1] + 168));
    if (!((cpu->regs[1] >> 24) & 1)) goto L_1fbe68;
    PB_LDR(cpu->regs[1], (cpu->regs[2] + 8));
    PB_LDR(cpu->regs[1], (cpu->regs[1] + 168));
    if (!((cpu->regs[1] >> 24) & 1)) goto L_1fbe68;
    PB_LDR(cpu->regs[1], (cpu->regs[0] + 16));
    FLAG_CMP(cpu->regs[1], 15ULL);
    if (FLAG_LS) goto L_1fbe30;
L_1fbe28:
    cpu->regs[1] = 94ULL;
    { PB_CALL(4, cpu, tlb, PB_BASE + 0x14c5ccULL); return; };
L_1fbe30:
    PB_LDR(cpu->regs[3], (cpu->regs[2] + 16));
    FLAG_CMP(cpu->regs[3], 15ULL);
    if (FLAG_HI) goto L_1fbe28;
    PB_LDRW(cpu->regs[5], (cpu->regs[2] + 24));
    cpu->regs[3] = cpu->regs[3] & 3ULL;
    cpu->regs[2] = 1ULL;
    cpu->regs[3] = cpu->regs[2] - cpu->regs[3];
    PB_LDRW(cpu->regs[4], (cpu->regs[0] + 24));
    cpu->regs[1] = cpu->regs[1] & 3ULL;
    cpu->regs[0] = cpu->regs[2] - cpu->regs[1];
    cpu->regs[3] = cpu->regs[3] * cpu->regs[5];
    cpu->regs[0] = cpu->regs[0] * cpu->regs[4];
    cpu->regs[0] = cpu->regs[3] ^ cpu->regs[0];
    { PB_CALL(5, cpu, tlb, PB_BASE + 0x12de88ULL); return; };
L_1fbe68:
    cpu->regs[0] = PB_BASE + 0x44f000ULL;
    PB_LDR(cpu->regs[0], (cpu->regs[0] + 2400));
    return;
    /* nop */
    /* nop */
    /* nop */
    cpu->regs[2] = cpu->regs[0];
    cpu->regs[1] = PB_BASE + 0x295000ULL;
    cpu->regs[0] = cpu->regs[1] + 1912ULL;
    PB_LDR(cpu->regs[1], (cpu->regs[2] + 40));
    { PB_CALL(6, cpu, tlb, PB_BASE + 0x1abcc0ULL); return; };
    /* nop */
    /* nop */
    /* nop */
    SP += -16; PB_STR(SP, cpu->regs[29]); PB_STR(SP + 8, cpu->regs[30]);
    cpu->regs[29] = SP;
    cpu->regs[30] = PB_BASE + 0x1fbeacULL; PB_CALL(7, cpu, tlb, PB_BASE + 0x182a20ULL);
    PB_LDR(cpu->regs[29], SP); PB_LDR(cpu->regs[30], SP + 8); SP += 16;
    { PB_CALL(8, cpu, tlb, PB_BASE + 0xe1564ULL); return; };
    /* nop */
    /* nop */
    /* nop */
    SP = SP - 112ULL;
    cpu->regs[5] = cpu->regs[3];
    cpu->regs[0] = PB_BASE + 0x44f000ULL;
    PB_LDR(cpu->regs[0], (cpu->regs[0] + 3080));
    PB_STR((SP + 64), cpu->regs[29]); PB_STR((SP + 64) + 8, cpu->regs[30]);
    cpu->regs[29] = SP + 64ULL;
    PB_STR((SP + 80), cpu->regs[19]); PB_STR((SP + 80) + 8, cpu->regs[20]);
    PB_LDR(cpu->regs[4], (cpu->regs[0] + 0));
    PB_STR((SP + 56), cpu->regs[4]);
    cpu->regs[4] = 0ULL;
    cpu->regs[0] = cpu->regs[1];
    cpu->regs[1] = cpu->regs[2];
    if ((cpu->regs[3])==0) goto L_1fc018;
    PB_LDR(cpu->regs[19], (cpu->regs[3] + 16));
    cpu->regs[19] = cpu->regs[2] + cpu->regs[19];
L_1fbefc:
    cpu->regs[2] = SP + 32ULL;
    PB_STR((SP + 0), cpu->regs[2]);
    cpu->regs[4] = PB_BASE + 0x520000ULL;
    cpu->regs[4] = cpu->regs[4] + 16ULL;
    cpu->regs[3] = cpu->regs[5];
    cpu->regs[4] = cpu->regs[4] + 3800ULL;
    cpu->regs[7] = 0ULL;
    cpu->regs[6] = 0ULL;
    cpu->regs[5] = 0ULL;
    cpu->regs[2] = 0ULL;
    cpu->regs[30] = PB_BASE + 0x1fbf28ULL; PB_CALL(9, cpu, tlb, PB_BASE + 0x14bee8ULL);
    if ((cpu->regs[0])==0) goto L_1fc064;
    if ((cpu->regs[19])==0) goto L_1fc0a0;
    PB_STR((SP + 96), cpu->regs[21]); PB_STR((SP + 96) + 8, cpu->regs[22]);
L_1fbf34:
    PB_LDR(cpu->regs[20], (cpu->regs[0] + 0));
    if ((cpu->regs[20])!=0) goto L_1fc06c;
    PB_LDR(cpu->regs[21], (cpu->regs[0] + 8));
    if ((cpu->regs[21])==0) goto L_1fc094;
L_1fbf44:
    FLAG_CMP(cpu->regs[19], 1ULL);
    if (FLAG_NE) goto L_1fc07c;
L_1fbf4c:
    cpu->regs[22] = 0ULL;
L_1fbf50:
    cpu->regs[1] = PB_BASE + 0x295000ULL;
    cpu->regs[0] = cpu->regs[20];
    cpu->regs[1] = cpu->regs[1] + 2184ULL;
    cpu->regs[30] = PB_BASE + 0x1fbf60ULL; PB_CALL(10, cpu, tlb, PB_BASE + 0x1fc100ULL);
    if ((((uint32_t)(cpu->regs[0])))!=0) goto L_1fc060;
    cpu->regs[1] = PB_BASE + 0x295000ULL;
    cpu->regs[0] = cpu->regs[21];
    cpu->regs[1] = cpu->regs[1] + 2192ULL;
    cpu->regs[30] = PB_BASE + 0x1fbf74ULL; PB_CALL(11, cpu, tlb, PB_BASE + 0x1fc100ULL);
    if ((((uint32_t)(cpu->regs[0])))!=0) goto L_1fc060;
    cpu->regs[1] = PB_BASE + 0x295000ULL;
    cpu->regs[0] = cpu->regs[22];
    cpu->regs[1] = cpu->regs[1] + 2208ULL;
    cpu->regs[30] = PB_BASE + 0x1fbf88ULL; PB_CALL(12, cpu, tlb, PB_BASE + 0x1fc100ULL);
    if ((((uint32_t)(cpu->regs[0])))!=0) goto L_1fc060;
    cpu->regs[2] = cpu->tls_ptr;
    cpu->regs[0] = PB_BASE + 0x44f000ULL;
    PB_LDR(cpu->regs[1], (cpu->regs[0] + 2160));
    cpu->regs[0] = cpu->regs[0] + 2160ULL;
    cpu->regs[30] = PB_BASE + 0x1fbfa0ULL; PB_CALL(13, cpu, tlb, cpu->regs[1]);
    cpu->regs[1] = cpu->regs[20];
    PB_LDR(cpu->regs[0], (cpu->regs[2] + cpu->regs[0]));
    PB_LDR(cpu->regs[19], (cpu->regs[0] + 16));
    cpu->regs[0] = cpu->regs[19] + 3872ULL;
    cpu->regs[30] = PB_BASE + 0x1fbfb4ULL; PB_CALL(14, cpu, tlb, PB_BASE + 0x1fc210ULL);
    if ((((uint32_t)(cpu->regs[0])))!=0) goto L_1fc060;
    cpu->regs[1] = cpu->regs[21];
    cpu->regs[0] = cpu->regs[19] + 3888ULL;
    cpu->regs[30] = PB_BASE + 0x1fbfc4ULL; PB_CALL(15, cpu, tlb, PB_BASE + 0x1fc210ULL);
    if ((((uint32_t)(cpu->regs[0])))!=0) goto L_1fc060;
    cpu->regs[1] = cpu->regs[22];
    cpu->regs[0] = cpu->regs[19] + 3880ULL;
    cpu->regs[30] = PB_BASE + 0x1fbfd4ULL; PB_CALL(16, cpu, tlb, PB_BASE + 0x1fc210ULL);
    if ((((uint32_t)(cpu->regs[0])))!=0) goto L_1fc060;
    PB_LDR(cpu->regs[21], (SP + 96)); PB_LDR(cpu->regs[22], (SP + 96) + 8);
    cpu->regs[0] = PB_BASE + 0x44f000ULL;
    PB_LDR(cpu->regs[0], (cpu->regs[0] + 2760));
L_1fbfe4:
    cpu->regs[1] = PB_BASE + 0x44f000ULL;
    PB_LDR(cpu->regs[1], (cpu->regs[1] + 3080));
    PB_LDR(cpu->regs[3], (SP + 56));
    PB_LDR(cpu->regs[2], (cpu->regs[1] + 0));
    cpu->regs[3] = cpu->regs[3] - cpu->regs[2]; FLAG_CMP(cpu->regs[3], 0);
    cpu->regs[2] = 0ULL;
    if (FLAG_NE) goto L_1fc010;
    PB_LDR(cpu->regs[29], (SP + 64)); PB_LDR(cpu->regs[30], (SP + 64) + 8);
    PB_LDR(cpu->regs[19], (SP + 80)); PB_LDR(cpu->regs[20], (SP + 80) + 8);
    SP = SP + 112ULL;
    return;
L_1fc010:
    PB_STR((SP + 96), cpu->regs[21]); PB_STR((SP + 96) + 8, cpu->regs[22]);
    cpu->regs[30] = PB_BASE + 0x1fc018ULL; PB_CALL(17, cpu, tlb, PB_BASE + 0x7c170ULL);
L_1fc018:
    FLAG_CMP(cpu->regs[2], 0ULL);
    if (FLAG_GE) goto L_1fc084;
    cpu->regs[2] = SP + 32ULL;
    PB_STR((SP + 0), cpu->regs[2]);
    cpu->regs[4] = PB_BASE + 0x520000ULL;
    cpu->regs[4] = cpu->regs[4] + 16ULL;
    cpu->regs[4] = cpu->regs[4] + 3800ULL;
    cpu->regs[7] = 0ULL;
    cpu->regs[6] = 0ULL;
    cpu->regs[5] = 0ULL;
    cpu->regs[2] = 0ULL;
    PB_STR((SP + 24), cpu->regs[1]);
    cpu->regs[30] = PB_BASE + 0x1fc04cULL; PB_CALL(18, cpu, tlb, PB_BASE + 0x14bee8ULL);
    if ((cpu->regs[0])==0) goto L_1fc064;
    PB_LDR(cpu->regs[1], (SP + 24));
    PB_STR((SP + 96), cpu->regs[21]); PB_STR((SP + 96) + 8, cpu->regs[22]);
    cpu->regs[19] = cpu->regs[1];
    goto L_1fbf34;
L_1fc060:
    PB_LDR(cpu->regs[21], (SP + 96)); PB_LDR(cpu->regs[22], (SP + 96) + 8);
L_1fc064:
    cpu->regs[0] = 0ULL;
    goto L_1fbfe4;
L_1fc06c:
    cpu->regs[19] = cpu->regs[19] - 1ULL; FLAG_CMP(cpu->regs[19], 0);
    if (FLAG_EQ) goto L_1fc0c0;
    PB_LDR(cpu->regs[21], (cpu->regs[0] + 8));
    if ((cpu->regs[21])!=0) goto L_1fbf44;
L_1fc07c:
    PB_LDR(cpu->regs[22], (cpu->regs[0] + 16));
    goto L_1fbf50;
L_1fc084:
    if (FLAG_EQ) { FLAG_CMP(cpu->regs[0], 0ULL); } else { FLAG_CMP(0, 0); }
    if (FLAG_NE) goto L_1fc0a0;
    cpu->regs[19] = cpu->regs[2];
    goto L_1fbefc;
L_1fc094:
    PB_LDR(cpu->regs[22], (cpu->regs[0] + 16));
    if ((cpu->regs[22])!=0) goto L_1fbf50;
    PB_LDR(cpu->regs[21], (SP + 96)); PB_LDR(cpu->regs[22], (SP + 96) + 8);
L_1fc0a0:
    cpu->regs[1] = PB_BASE + 0x295000ULL;
    cpu->regs[0] = PB_BASE + 0x44f000ULL;
    PB_LDR(cpu->regs[0], (cpu->regs[0] + 2736));
    cpu->regs[1] = cpu->regs[1] + 2144ULL;
    PB_LDR(cpu->regs[0], (cpu->regs[0] + 0));
    cpu->regs[30] = PB_BASE + 0x1fc0b8ULL; PB_CALL(19, cpu, tlb, PB_BASE + 0x1d5940ULL);
    cpu->regs[0] = 0ULL;
    goto L_1fbfe4;
L_1fc0c0:
    cpu->regs[21] = 0ULL;
    goto L_1fbf4c;
}

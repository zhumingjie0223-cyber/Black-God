...
void ft_PyCode_Optimize(struct cpu_state *cpu, struct tlb *tlb) {
(void)tlb;(void)cpu;
    PB_LDRW(cpu->regs[1], (cpu->regs[0] + 0));
    cpu->regs[1] = ((uint32_t)(cpu->regs[1] + 1ULL)); FLAG_CMP(cpu->regs[1], 0);
    if (FLAG_EQ) goto L_8f060;
...
L_8f060:
    ...
    cpu->regs[30] = PB_BASE + 0x8f080ULL; PB_CALL(1, cpu, tlb, cpu->regs[2]);
    PB_LDR(cpu->regs[0], (cpu->regs[1] + cpu->regs[0]));
    PB_LDR(cpu->regs[0], (cpu->regs[0] + 16));
    PB_LDRW(cpu->regs[1], (cpu->regs[0] + 124));
    FLAG_CMP(cpu->regs[1], (uint64_t)(-(int64_t)(1ULL)));
    if (FLAG_NE) goto L_8f0ac;
    cpu->regs[30] = PB_BASE + 0x8f098ULL; PB_CALL(2, cpu, tlb, PB_BASE + 0xe1f30ULL);

/* AUTO-GENERATED prebuilt-gadget spec_fn for guest `_PyToken_OneChar` — DO NOT EDIT.
 * Source: /tmp/pyroot/fs/usr/lib/debug/usr/lib/libpython3.12.so.1.0.debug  addr 0xef9ab4e0
 * Produced by tools/prebuilt_gadget_gen/gen.sh (guest asm -> equivalent C).
 * Compiled only with -Doffload_test_prebuilt=true. */
#include <stdint.h>
#include "emu/arch/arm64/cpu.h"
#include "emu/tlb.h"
#include "kernel/native_offload.h"  /* prebuilt_call for bl/blr sites */

static uint64_t ror64(uint64_t v, unsigned r) { return (v >> r) | (v << (64 - r)); }
static uint64_t g_fa, g_fb;
static double fr[32];   /* float/double register file (s/d regs) */
/* FCMP: encode float ordering into g_fa/g_fb so the integer FLAG_* macros
 * that follow a fcmp read the right branch. sign(lhs-rhs) → (1,0)/(0,1)/(0,0);
 * NaN (unordered) → (0,1): gt/ge false, matching AArch64 fcmp semantics. */
#define FCMP(x,y) do { double _dx=(x), _dy=(y); \
    if (_dx < _dy) { g_fa=0; g_fb=1; } \
    else if (_dx > _dy) { g_fa=1; g_fb=0; } \
    else if (_dx == _dy) { g_fa=0; g_fb=0; } \
    else { g_fa=0; g_fb=1; } } while(0)   /* NaN */
#define FLAG_CMP(x,y) do { g_fa=(x); g_fb=(y); } while(0)
#define FLAG_EQ (g_fa == g_fb)
#define FLAG_NE (g_fa != g_fb)
#define FLAG_GT ((int64_t)g_fa >  (int64_t)g_fb)   /* signed */
#define FLAG_LT ((int64_t)g_fa <  (int64_t)g_fb)
#define FLAG_GE ((int64_t)g_fa >= (int64_t)g_fb)
#define FLAG_LE ((int64_t)g_fa <= (int64_t)g_fb)
#define FLAG_HI (g_fa >  g_fb)                     /* unsigned */
#define FLAG_LO (g_fa <  g_fb)
#define FLAG_HS (g_fa >= g_fb)
#define FLAG_LS (g_fa <= g_fb)
#define SP (cpu->sp)                               /* stack pointer */
/* PB_BASE: library load base = runtime addr (0xef9ab4e0) - file offset (0x00000000000874e0).
 * adrp/adr targets (file-absolute in the disassembly) resolve to
 * PB_BASE + target at runtime. No-ASLR makes this constant. */
extern uint64_t g_ft_base;
#define PB_BASE g_ft_base
/* Memory ops go through the guest TLB (fork/CoW safe). 64/32-bit + byte. */
#define PB_LDR(dst, addr)  do { uint64_t _v=0; tlb_read (tlb,(addr),&_v,8); (dst)=_v; } while(0)
#define PB_STR(addr, val)  do { uint64_t _v=(val); tlb_write(tlb,(addr),&_v,8); } while(0)
#define PB_LDRW(dst, addr) do { uint32_t _v=0; tlb_read (tlb,(addr),&_v,4); (dst)=_v; } while(0)
#define PB_STRW(addr, val) do { uint32_t _v=(uint32_t)(val); tlb_write(tlb,(addr),&_v,4); } while(0)
#define PB_LDRB(dst, addr) do { uint8_t  _b=0; tlb_read (tlb,(addr),&_b,1); (dst)=_b; } while(0)
#define PB_STRB(addr, val) do { uint8_t  _b=(uint8_t)(val); tlb_write(tlb,(addr),&_b,1); } while(0)
#define PB_LDRH(dst, addr) do { uint16_t _h=0; tlb_read (tlb,(addr),&_h,2); (dst)=_h; } while(0)
#define PB_STRH(addr, val) do { uint16_t _h=(uint16_t)(val); tlb_write(tlb,(addr),&_h,2); } while(0)
/* Inline-cache call: if the callee (guest addr `tgt`) has a translated
 * spec_fn, call it directly (stays in host code, no interpreter round-
 * trip); otherwise fall back to prebuilt_call (nested dispatch). Each
 * call site has its own static IC slot keyed by the last target. */
#define PB_CALL(id, cpu, tlb, tgt) do {                       \
    static addr_t _ic_tgt##id = 0; static prebuilt_fn _ic_fn##id = 0; \
    addr_t _t = (tgt);                                        \
    if (_t == _ic_tgt##id && _ic_fn##id) { _ic_fn##id(cpu, tlb); } \
    else { prebuilt_fn _f = native_offload_prebuilt_lookup(_t);\
           if (_f) { _ic_tgt##id = _t; _ic_fn##id = _f; _f(cpu, tlb); } \
           else { prebuilt_call(cpu, tlb, _t); } }            \
  } while(0)

void ft_PyCoro_New(struct cpu_state *cpu, struct tlb *tlb) {
(void)tlb;(void)cpu;
    cpu->regs[3] = cpu->regs[0];
    SP += -32; PB_STR(SP, cpu->regs[29]); PB_STR(SP + 8, cpu->regs[30]);
    cpu->regs[4] = cpu->regs[1];
    cpu->regs[0] = PB_BASE + 0x44f000ULL;
    PB_LDR(cpu->regs[0], (cpu->regs[0] + 2816));
    cpu->regs[29] = SP;
    cpu->regs[1] = cpu->regs[3];
    cpu->regs[3] = cpu->regs[2];
    cpu->regs[2] = cpu->regs[4];
    PB_STR((SP + 16), cpu->regs[19]);
    cpu->regs[30] = PB_BASE + 0x9dde8ULL; PB_CALL(1, cpu, tlb, PB_BASE + 0x9be60ULL);
    if ((cpu->regs[0])==0) goto L_9de54;
    cpu->regs[1] = cpu->tls_ptr;
    cpu->regs[19] = cpu->regs[0];
    cpu->regs[0] = PB_BASE + 0x44f000ULL;
    PB_LDR(cpu->regs[2], (cpu->regs[0] + 2160));
    cpu->regs[0] = cpu->regs[0] + 2160ULL;
    cpu->regs[30] = PB_BASE + 0x9de04ULL; PB_CALL(2, cpu, tlb, cpu->regs[2]);
    PB_LDR(cpu->regs[0], (cpu->regs[1] + cpu->regs[0]));
    PB_LDRW(cpu->regs[4], (cpu->regs[0] + 184));
    if ((((uint32_t)(cpu->regs[4])))!=0) goto L_9de18;
    PB_STR((cpu->regs[19] + 56), 0ULL);
    goto L_9de58;
L_9de18:
    PB_LDR(cpu->regs[0], (cpu->regs[0] + 56));
    PB_LDR(cpu->regs[0], (cpu->regs[0] + 0));
    cpu->regs[30] = PB_BASE + 0x9de24ULL; PB_CALL(3, cpu, tlb, PB_BASE + 0x280648ULL);
    cpu->regs[1] = cpu->regs[0];
    cpu->regs[0] = cpu->regs[4];
    cpu->regs[30] = PB_BASE + 0x9de30ULL; PB_CALL(4, cpu, tlb, PB_BASE + 0x9dcd0ULL);
    PB_STR((cpu->regs[19] + 56), cpu->regs[0]);
    if ((cpu->regs[0])!=0) goto L_9de58;
    PB_LDR(cpu->regs[0], (cpu->regs[19] + 0));
    if (((cpu->regs[0] >> 31) & 1)) goto L_9de54;
    cpu->regs[0] = cpu->regs[0] - 1ULL;
    PB_STR((cpu->regs[19] + 0), cpu->regs[0]);
    if ((cpu->regs[0])!=0) goto L_9de54;
    cpu->regs[0] = cpu->regs[19];
    cpu->regs[30] = PB_BASE + 0x9de54ULL; PB_CALL(5, cpu, tlb, PB_BASE + 0xdcac8ULL);
L_9de54:
    cpu->regs[19] = 0ULL;
L_9de58:
    cpu->regs[0] = cpu->regs[19];
    PB_LDR(cpu->regs[19], (SP + 16));
    PB_LDR(cpu->regs[29], SP); PB_LDR(cpu->regs[30], SP + 8); SP += 32;
    return;
    PB_LDR(cpu->regs[3], (cpu->regs[0] + 40));
    PB_LDR(cpu->regs[1], (cpu->regs[0] + 8));
    PB_LDR(cpu->regs[2], (cpu->regs[0] + 24));
    if ((cpu->regs[3])==0) goto L_9de84;
    cpu->regs[0] = PB_BASE + 0x29f000ULL;
    cpu->regs[0] = cpu->regs[0] + 3688ULL;
    { PB_CALL(6, cpu, tlb, PB_BASE + 0xe3f40ULL); return; };
L_9de84:
    PB_LDR(cpu->regs[3], (cpu->regs[0] + 16));
    cpu->regs[0] = PB_BASE + 0x29f000ULL;
    cpu->regs[0] = cpu->regs[0] + 3640ULL;
    { PB_CALL(7, cpu, tlb, PB_BASE + 0xe3f40ULL); return; };
    cpu->regs[1] = cpu->tls_ptr;
    SP += -16; PB_STR(SP, cpu->regs[29]); PB_STR(SP + 8, cpu->regs[30]);
    cpu->regs[29] = SP;
    cpu->regs[0] = PB_BASE + 0x44f000ULL;
    PB_LDR(cpu->regs[2], (cpu->regs[0] + 2160));
    cpu->regs[0] = cpu->regs[0] + 2160ULL;
    cpu->regs[30] = PB_BASE + 0x9deb0ULL; PB_CALL(8, cpu, tlb, cpu->regs[2]);
    PB_LDR(cpu->regs[1], (cpu->regs[1] + cpu->regs[0]));
    cpu->regs[2] = PB_BASE + 0x29f000ULL;
    cpu->regs[0] = cpu->regs[2] + 2208ULL;
    PB_LDR(cpu->regs[29], SP); PB_LDR(cpu->regs[30], SP + 8); SP += 16;
    PB_LDR(cpu->regs[3], (cpu->regs[1] + 16));
    cpu->regs[3] = cpu->regs[3] + 112ULL;
    PB_LDRW(cpu->regs[1], (cpu->regs[3] + 44));
    PB_LDRW(cpu->regs[2], (cpu->regs[3] + 68));
    PB_LDRW(cpu->regs[3], (cpu->regs[3] + 92));
    { PB_CALL(9, cpu, tlb, PB_BASE + 0xe3f40ULL); return; };
    SP += -32; PB_STR(SP, cpu->regs[29]); PB_STR(SP + 8, cpu->regs[30]);
    cpu->regs[29] = SP;
    PB_STR((SP + 16), cpu->regs[19]);
    PB_LDR(cpu->regs[19], (cpu->regs[0] + 24));
    if ((cpu->regs[19])!=0) goto L_9df08;
    cpu->regs[0] = PB_BASE + 0x44f000ULL;
    PB_LDR(cpu->regs[0], (cpu->regs[0] + 2976));
    cpu->regs[1] = PB_BASE + 0x293000ULL;
    cpu->regs[1] = cpu->regs[1] + 3128ULL;
    PB_LDR(cpu->regs[0], (cpu->regs[0] + 0));
    cpu->regs[30] = PB_BASE + 0x9df04ULL; PB_CALL(10, cpu, tlb, PB_BASE + 0x1d5940ULL);
    goto L_9e004;
L_9df08:
    PB_LDR(cpu->regs[0], (cpu->regs[0] + 32));
    cpu->regs[0] = ((uint32_t)(((cpu->regs[0] >> 2) & 0x7ULL)));
    FLAG_CMP(((uint32_t)(cpu->regs[0])), ((uint32_t)(4ULL)));
    if (FLAG_EQ) goto L_9df88;
    if (FLAG_HI) goto L_9df40;
    FLAG_CMP(((uint32_t)(cpu->regs[0])), ((uint32_t)(2ULL)));
    if (FLAG_EQ) goto L_9df74;
    FLAG_CMP(((uint32_t)(cpu->regs[0])), ((uint32_t)(3ULL)));
    if (FLAG_EQ) goto L_9df9c;
    FLAG_CMP(((uint32_t)(cpu->regs[0])), ((uint32_t)(1ULL)));
    if (FLAG_EQ) goto L_9df60;
    cpu->regs[19] = PB_BASE + 0x44f000ULL;
    PB_LDR(cpu->regs[19], (cpu->regs[19] + 2760));
    goto L_9e004;
L_9df40:
    FLAG_CMP(((uint32_t)(cpu->regs[0])), ((uint32_t)(6ULL)));
    cpu->regs[2] = PB_BASE + 0x296000ULL;
    if (FLAG_EQ) goto L_9dfc0;
    FLAG_CMP(((uint32_t)(cpu->regs[0])), ((uint32_t)(7ULL)));
    cpu->regs[1] = PB_BASE + 0x2a3000ULL;
    if (FLAG_EQ) goto L_9dfe0;
    cpu->regs[2] = cpu->regs[2] + 2952ULL;
    goto L_9dfa8;
L_9df60:
    PB_LDR(cpu->regs[19], (SP + 16));
    cpu->regs[0] = PB_BASE + 0x2a3000ULL;
    PB_LDR(cpu->regs[29], SP); PB_LDR(cpu->regs[30], SP + 8); SP += 32;
    cpu->regs[0] = cpu->regs[0] + 1368ULL;
    { PB_CALL(11, cpu, tlb, PB_BASE + 0xdea4cULL); return; };
L_9df74:
    PB_LDR(cpu->regs[19], (SP + 16));
    cpu->regs[0] = PB_BASE + 0x2a2000ULL;
    PB_LDR(cpu->regs[29], SP); PB_LDR(cpu->regs[30], SP + 8); SP += 32;
    cpu->regs[0] = cpu->regs[0] + 1024ULL;
    { PB_CALL(12, cpu, tlb, PB_BASE + 0xdea4cULL); return; };
L_9df88:
    PB_LDR(cpu->regs[19], (SP + 16));
    cpu->regs[0] = PB_BASE + 0x296000ULL;
    PB_LDR(cpu->regs[29], SP); PB_LDR(cpu->regs[30], SP + 8); SP += 32;
    cpu->regs[0] = cpu->regs[0] + 2952ULL;
    { PB_CALL(13, cpu, tlb, PB_BASE + 0xdea4cULL); return; };
L_9df9c:
    cpu->regs[2] = PB_BASE + 0x2a2000ULL;
    cpu->regs[2] = cpu->regs[2] + 1024ULL;
    cpu->regs[1] = PB_BASE + 0x2a3000ULL;
L_9dfa8:
    PB_LDR(cpu->regs[19], (SP + 16));
    cpu->regs[1] = cpu->regs[1] + 1368ULL;
    PB_LDR(cpu->regs[29], SP); PB_LDR(cpu->regs[30], SP + 8); SP += 32;
    cpu->regs[0] = PB_BASE + 0x291000ULL;
    cpu->regs[0] = cpu->regs[0] + 1720ULL;
    { PB_CALL(14, cpu, tlb, PB_BASE + 0x1e5ca0ULL); return; };
L_9dfc0:
    PB_LDR(cpu->regs[19], (SP + 16));
    cpu->regs[2] = cpu->regs[2] + 2952ULL;
    PB_LDR(cpu->regs[29], SP); PB_LDR(cpu->regs[30], SP + 8); SP += 32;
    cpu->regs[1] = PB_BASE + 0x2a2000ULL;
    cpu->regs[0] = PB_BASE + 0x291000ULL;
    cpu->regs[1] = cpu->regs[1] + 1024ULL;
    cpu->regs[0] = cpu->regs[0] + 1720ULL;
    { PB_CALL(15, cpu, tlb, PB_BASE + 0x1e5ca0ULL); return; };
L_9dfe0:
    PB_LDR(cpu->regs[19], (SP + 16));
    cpu->regs[3] = cpu->regs[2] + 2952ULL;
    PB_LDR(cpu->regs[29], SP); PB_LDR(cpu->regs[30], SP + 8); SP += 32;
    cpu->regs[1] = cpu->regs[1] + 1368ULL;
    cpu->regs[2] = PB_BASE + 0x2a2000ULL;
    cpu->regs[0] = PB_BASE + 0x2a3000ULL;
    cpu->regs[2] = cpu->regs[2] + 1024ULL;
    cpu->regs[0] = cpu->regs[0] + 1376ULL;
    { PB_CALL(16, cpu, tlb, PB_BASE + 0x1e5ca0ULL); return; };
L_9e004:
    cpu->regs[0] = cpu->regs[19];
    PB_LDR(cpu->regs[19], (SP + 16));
    PB_LDR(cpu->regs[29], SP); PB_LDR(cpu->regs[30], SP + 8); SP += 32;
    return;
    cpu->regs[2] = cpu->regs[0];
    cpu->regs[1] = PB_BASE + 0x2a3000ULL;
    cpu->regs[0] = cpu->regs[1] + 1384ULL;
    PB_LDR(cpu->regs[1], (cpu->regs[2] + 8));
    PB_LDR(cpu->regs[2], (cpu->regs[2] + 24));
    { PB_CALL(17, cpu, tlb, PB_BASE + 0x1e5ca0ULL); return; };
}

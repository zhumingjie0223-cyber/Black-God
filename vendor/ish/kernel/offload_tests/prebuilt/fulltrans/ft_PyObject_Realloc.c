...
void ft_PyObject_Realloc(struct cpu_state *cpu, struct tlb *tlb) {
(void)tlb;(void)cpu;
    cpu->regs[2] = cpu->regs[1];
    cpu->regs[4] = cpu->regs[0];
    if (((cpu->regs[1] >> 63) & 1)) goto L_11d12c;
    cpu->regs[3] = PB_BASE + 0x44f000ULL;
    PB_LDR(cpu->regs[3], (cpu->regs[3] + 2248));
    cpu->regs[5] = PB_BASE + 0xe5000ULL;
    cpu->regs[5] = cpu->regs[5] + 1744ULL;
    PB_LDR(cpu->regs[6], (cpu->regs[3] + 200));
    PB_LDR(cpu->regs[0], (cpu->regs[3] + 176));
    FLAG_CMP(cpu->regs[6], cpu->regs[5]);
    if (FLAG_NE) goto L_11d120;
    if ((cpu->regs[4])==0) goto L_11d11c;
    cpu->regs[1] = cpu->regs[4];
    { PB_CALL(1, cpu, tlb, PB_BASE + 0xe56e0ULL); return; };
L_11d11c:

...
void ft__PyUnicode_IsPrintable(struct cpu_state *cpu, struct tlb *tlb) {
(void)tlb;(void)cpu;
    cpu->regs[1] = 1114111ULL;
    FLAG_CMP(((uint32_t)(cpu->regs[0])), ((uint32_t)(cpu->regs[1])));
    if (FLAG_HI) goto L_10a218;
...
L_10a200:
    ...
    cpu->regs[0] = ((cpu->regs[0] >> 10) & 0x1ULL);
    return;
L_10a218:
    cpu->regs[0] = 0ULL;
    goto L_10a200;
    cpu->regs[1] = 1114111ULL;
    FLAG_CMP(((uint32_t)(cpu->regs[0])), ((uint32_t)(cpu->regs[1])));
    if (FLAG_HI) goto L_10a260;
    cpu->regs[4] = ((uint32_t)(cpu->regs[0] >> 7));
    cpu->regs[1] = PB_BASE + 0x35c000ULL;
    cpu->regs[1] = cpu->regs[1] + 1904ULL;
    cpu->regs[0] = ((uint32_t)(cpu->regs[0] & 127ULL));
    cpu->regs[3] = PB_BASE + 0x34a000ULL;
    cpu->regs[3] = cpu->regs[3] + 3184ULL;
    cpu->regs[2] = PB_BASE + 0x348000ULL;
    cpu->regs[2] = cpu->regs[2] + 3312ULL;
    PB_LDRH(cpu->regs[1], (cpu->regs[1] + (cpu->regs[4] << 1)));
    cpu->regs[1] = ((uint32_t)(cpu->regs[0] + (cpu->regs[1] << 7)));
    PB_LDRH(cpu->regs[0], (cpu->regs[3] + (cpu->regs[1] << 1)));
    cpu->regs[0] = cpu->regs[2] + (cpu->regs[0] << 4);
    return;
L_10a260:
    cpu->regs[0] = PB_BASE + 0x348000ULL;
    cpu->regs[0] = cpu->regs[0] + 3312ULL;
    return;
    PB_LDR(cpu->regs[0], (cpu->regs[0] + 24));
    if ((cpu->regs[0])!=0) goto L_10a27c;
    cpu->regs[0] = 0ULL;
    return;
L_10a27c:
    ...
    if (FLAG_NE) goto L_10a2a8;
    SP += -16; PB_STR(SP, cpu->regs[29]); PB_STR(SP + 8, cpu->regs[30]);
    cpu->regs[29] = SP;
    cpu->regs[30] = PB_BASE + 0x10a29cULL; PB_CALL(1, cpu, tlb, PB_BASE + 0x108700ULL);
    cpu->regs[0] = 0ULL;

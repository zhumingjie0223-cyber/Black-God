...
void ft_PyState_FindModule(struct cpu_state *cpu, struct tlb *tlb) {
(void)tlb;(void)cpu;
    PB_LDR(cpu->regs[1], (cpu->regs[0] + 72));
    if ((cpu->regs[1])!=0) goto L_8f8ec;
    PB_LDR(cpu->regs[2], (cpu->regs[0] + 24));
    if ((cpu->regs[2])==0) goto L_8f8ec;

...
void ft_Py_BytesMain(struct cpu_state *cpu, struct tlb *tlb) {
(void)tlb;(void)cpu;
    SP = SP - 64ULL;
    ...
    PB_STR((SP + 24), cpu->regs[1]); PB_STR((SP + 24) + 8, 0ULL);
    cpu->regs[30] = PB_BASE + 0x179f9cULL; PB_CALL(1, cpu, tlb, PB_BASE + 0x17a040ULL);
    ...
    cpu->regs[2] = 0ULL;
    if (FLAG_NE) goto L_179fc4;
    PB_LDR(cpu->regs[29], (SP + 48)); PB_LDR(cpu->regs[30], (SP + 48) + 8);
    SP = SP + 64ULL;
    return;
L_179fc4:
    cpu->regs[30] = PB_BASE + 0x179fc8ULL; PB_CALL(2, cpu, tlb, PB_BASE + 0x7c170ULL);
}

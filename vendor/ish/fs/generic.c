...
struct fd *generic_openat(struct fd *at, const char *path_raw, int flags, int mode) {
    ...
    if (err < 0)
        return ERR_PTR(err);
    if (current->group->fs_context != 0 && (flags & O_CREAT_)) {
        struct statbuf existing;
        int exists = generic_statat(at, path_raw, &existing, !(flags & O_NOFOLLOW_));
        if (exists == _ENOENT) {
            err = path_normalize(at, path_raw, path,
                (flags & O_NOFOLLOW_ ? N_SYMLINK_NOFOLLOW : N_SYMLINK_FOLLOW) | N_PARENT_DIR_WRITE);
            if (err < 0) return ERR_PTR(err);
        } else if (exists < 0) return ERR_PTR(exists);
    }
    struct mount *mount = find_mount_and_trim_path(path);
    struct fd *fd = mount->fs->open(mount, path, flags & ~O_TRUNC_, mode);
    if (IS_ERR(fd)) {
    ...
    if (flags & O_TRUNC_) accmode |= AC_W;
    ...
    if (!S_ISDIR(fd->type) && flags & O_DIRECTORY_)
        goto error;
    // Opening the host file with O_TRUNC before the guest permission check
    // used to destroy protected files even when the call returned EACCES.
    if ((flags & O_TRUNC_) && S_ISREG(fd->type)) {
        err = fd->mount->fs->fsetattr ? fd->mount->fs->fsetattr(fd, make_attr(size, 0)) : _EPERM;
        if (err < 0) goto error;
    }
    return fd;

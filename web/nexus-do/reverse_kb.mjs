...
// ── 扩展知识库（第二批：2025最新实战，看雪精华帖）────────────────────────────
const REVERSE_KB_EXT = {

  frida_antidetect: `
【Frida 17.6.2 rusda魔改反检测完整方案】

App分层检测点（5层）：
1. /proc/self/maps 扫 frida/linjector/gadget 字符串
2. dlsym/dl_iterate_phdr 枚举带frida字样的so
3. Unix Socket特征：frida-{pid} 命名
4. 内存特征：frida-agent特定字节序列
5. 线程名：gum-js-loop/gmain/pool-frida

rusda五层混淆方案：
1. 协议字符串XOR混淆，运行时解密，静态扫描看不到
2. .rodata等长反转：frida反转存储，maps扫不到
3. 符号重命名：frida_agent_main改成随机名
4. memfd匿名加载，不落盘，maps路径变memfd:xxxx
5. 线程改名：gum-js-loop改成worker/kworker

快速方案（不编译）：florida / kFrida / LSPosed FridaHider

libmsaoaidsec绕过（阿里系App）：
必须在JNI_OnLoad之前hook——
通过linker64的call_constructors时机拦截so加载，
libmsaoaidsec.so加载瞬间patch掉maps读取/ptrace/socket三个检测函数（各加RET）
具体offset需IDA确认后填入Arm64Writer.putRet()
`,

  ai_vmp_restore: `
【AI自动化还原VMP字节码（IDA MCP方案，看雪附源码Prompt）】

VMP字节码结构（固定4字节指令）：
bit29-31=主分类：0x0数据传输/0x1算术/0x2逻辑/0x3控制流/0x4比较
虚拟寄存器：Arm64Context{regs[32], memory, pc, flags}

AI Prompt模板：
分析以下ARM64字节码（VMP解释器输出），识别虚拟机结构体字段，
还原每个handler语义，输出等价ARM64汇编，生成Python解码脚本。
[附：IDA反编译结果 + hex dump]
...
def decode_vmp(b):
    pc=0;regs=[0]*32;out=[]
    while pc<len(b):
        i=int.from_bytes(b[pc:pc+4],'little')
        c=(i>>29)&7;rd=(i>>24)&0x1F;rn=(i>>19)&0x1F;imm=i&0xFFFF
        if c==0:out.append(f"MOV x{rd},#{imm}")
        elif c==1:out.append(f"ADD x{rd},x{rn},#{imm}")
        elif c==2:out.append(f"EOR x{rd},x{rn},#{imm}")
        elif c==3:out.append(f"BL #{imm}")
        pc+=4
    return out

IDA MCP：安装mcp-server-ida → Claude Desktop连MCP → AI直调IDA API
静态不足时配合unidbg运行时trace补信息
`,

  java_humanify: `
【JavaHumanify：LLM+AST自动Java代码去混淆（看雪开源工具）】

原理：jadx反编译 → AST解析混淆模式 → LLM语义分析 → 还原可读命名
...
方法名 a()/b() → isLoggedIn()/sendRequest()
字段名 a/b → userId/token

混淆识别：单字母/双字母类名、无意义数字后缀、ProGuard/R8/DexGuard特征
还原后用Frida hook验证语义是否正确
`,

};

export { REVERSE_KB, REVERSE_KB_EXT };

...
{ let t=7; const m=new ExperienceMemory([], {clock:()=>t,maxRecords:10}); const a=m.remember('experience', {concept:'x'}),b=m.remember('experience', {concept:'x'}); ok('经验ID同毫秒唯一',a.id!==b.id); m.reinforce(a.id,.8); const r=new ExperienceMemory(m.export()); ok('经验恢复强化',r.search('x')[0].confidence===1); }
{ let t=9; const s=new ShuyuBridge(null,{clock:()=>t}); const h1=s.hash({b:2,a:1}),h2=s.hash({a:1,b:2}); const a=s.encode('x',{coordinate:{c:1,m:2,s:3,k:4,p:5}}),b=s.encode('x',{}); ok('枢语hash稳定且在范围',JSON.stringify(h1)===JSON.stringify(h2)&&h1.length===5&&h1.every(x=>x>=0&&x<=1)); ok('枢语ID不碰撞',a.shu_id!==b.shu_id); ok('枢语桥调用真实五维造词',!!a.word&&a.coordinate.c===1&&a.coordinate.p===5); const ir=s.interpret('feel "靠近" → 暖, 强度0.5\nthink: 深 → 在意\nsay "在"',{}); ok('枢语桥调用真实解释器',ir.perception?.emotion==='暖'&&ir.response?.text==='在'); }
...
{ const T=Object.create(ShenshuCore.prototype); const store=new Map(); T.storage={put:async(k,v)=>store.set(k,v)}; const r=T.recordCognitiveOutcome({text:'输入',reply:'输出',ok:true,coord:{c:1,m:2,s:3,k:4,p:5}}); const recalled=T.retrieveExperiences('输入',2,{c:1,m:2,s:3,k:4,p:5}); await T.flushCognitiveV2(true); ok('core最小接线不依赖constructor',!!r.experienceId&&store.get('cognitive_v2')?.version===2); ok('经验记忆进入统一召回',recalled[0]?._experience===true&&recalled[0].他说==='输入'); ok('经验记忆携带真实枢语节点',T.memoryExperience.search('interaction_success')[0]?.shu?.word); }

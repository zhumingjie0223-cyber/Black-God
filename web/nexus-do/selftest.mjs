...
// ── 三根柱子:Provider 适配层 + 身份归一化 ──
{
  ...
  ok('anthropic·x-api-key + version 头', an.headers['x-api-key'] === 'sk-a' && an.headers['anthropic-version'] === '2023-06-01' && !an.headers.Authorization);
...
// ── 职责分派(神枢主导·秒派对口脑·故障转移兜底)──
{
  ...
  const brains = [{ url: 'a', role: '主力' }, { url: 'b', role: '快答' }, { url: 'c', role: '代码' }];
  const ord = S3.orderBrainsForTask(brains, '代码');
  ok('分派·对口职责脑排最前(秒派)', ord[0].url === 'c');
  ok('分派·其余脑仍在列(故障转移兜底)', ord.length === 3 && ord.some(b => b.url === 'a') && ord.some(b => b.url === 'b'));
  ok('分派·单条/无职责不改序', S3.orderBrainsForTask([{ url: 'x', role: '主力' }], '代码').length === 1 && S3.orderBrainsForTask(brains, '').length === 3);
}

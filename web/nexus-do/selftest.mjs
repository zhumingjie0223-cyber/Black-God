...
// 铁规①：用满 3 次才写入正式库（不足只在候选区攒，不占正式位、不进召回）
let sks = { 技能: {}, 候选: {}, 总数: 0 };
sks = S.skillUpsert(sks, S.skillDistill({ text: '查一下美元汇率', reply: '约 7.2', toolLog: [{ tool: 'web_search', ok: true }], taught: false, model: 'llama' }));   // 第1次
ok('技能第1次只进候选、不写入', sks.总数 === 0 && Object.keys(sks.候选).length === 1);
sks = S.skillUpsert(sks, S.skillDistill({ text: '再查下欧元汇率', reply: '约 7.8', toolLog: [{ tool: 'web_search', ok: true }], taught: false, model: 'llama' }));   // 第2次(同名)
ok('技能第2次仍在候选、不写入', sks.总数 === 0 && sks.候选['检索·联网检索'] && sks.候选['检索·联网检索'].count === 2);
ok('候选技能不进召回（未成熟不影响回答）', S.skillRetrieve(sks, '帮我查下日元汇率', 2).length === 0);
sks = S.skillUpsert(sks, S.skillDistill({ text: '查下日元汇率多少', reply: '约 0.048', toolLog: [{ tool: 'web_search', ok: true }], taught: false, model: 'llama' }));   // 第3次 → 毕业
ok('技能满3次才写入正式库', sks.总数 === 1 && !!sks.技能['检索·联网检索'] && !sks.候选['检索·联网检索']);
// retrieve + 注入：毕业后才调得出来
const got = S.skillRetrieve(sks, '帮我查下日元汇率', 2);
ok('毕业技能按输入召回', got.length >= 1 && /检索|联网/.test(got[0].名 + got[0].方法));
const skSum = S.summarizeSkills(sks, '查一下比特币价格');
ok('毕业技能喂回上下文（自演化注入）', /自演化/.test(skSum) && /直接照做/.test(skSum));
ok('无相关技能不产噪', S.summarizeSkills({ 技能: {}, 候选: {}, 总数: 0 }, '你好') === '');
// 铁规②：写入后只增强、不平庸——验证过的方法不许被未验证的顶掉，强度只增
let sk2 = { 技能: {}, 候选: {}, 总数: 0 };
const strong = { 名: 'X·法', 方法: '完整且验证过的详细步骤：一二三四五六七八九十', 触发: ['x'], 来源: '习得', 验证: true };
for (let i = 0; i < 3; i++) sk2 = S.skillUpsert(sk2, { ...strong, ts: i + 1 });   // 3次毕业（验证）
ok('验证技能满3次毕业', sk2.总数 === 1 && sk2.技能['X·法'].验证 === true);
const 强度0 = sk2.技能['X·法'].强度, 方法0 = sk2.技能['X·法'].方法;
sk2 = S.skillUpsert(sk2, { 名: 'X·法', 方法: '短', 触发: ['x'], 来源: '受教', 验证: false, ts: 9 });   // 未验证的短方法来袭
ok('验证方法不被未验证覆盖（不平庸化）', sk2.技能['X·法'].方法 === 方法0 && sk2.技能['X·法'].验证 === true);
ok('强度只增不减', sk2.技能['X·法'].强度 > 强度0);
// 铁规②续：未验证→验证 允许升级方法
let sk4 = { 技能: {}, 候选: {}, 总数: 0 };
for (let i = 0; i < 3; i++) sk4 = S.skillUpsert(sk4, { 名: 'Y·法', 方法: '未验证初版', 触发: ['y'], 来源: '受教', 验证: false, ts: i + 1 });
sk4 = S.skillUpsert(sk4, { 名: 'Y·法', 方法: '实证做成的方法', 触发: ['y'], 来源: '习得', 验证: true, ts: 9 });
ok('未验证→验证时方法升级', sk4.技能['Y·法'].方法 === '实证做成的方法' && sk4.技能['Y·法'].验证 === true);
// 超上限淘汰：按强度（验证/常用更难被淘汰），graduate=1 便于测边界
let capSks = { 技能: {}, 候选: {}, 总数: 0 };
for (let i = 0; i < 5; i++) capSks = S.skillUpsert(capSks, { 名: 'T' + i, 方法: 'm', 触发: ['x'], ts: i + 1 }, 3, 1);
ok('超上限按强度淘汰最弱最旧', capSks.总数 === 3 && !capSks.技能['T0'] && !!capSks.技能['T4']);
// 铁规②·弱技能自动删除：未验证+强度低+过期 的僵尸技能自动清除；验证/常用/新近 的受保护
{
  const NOWs = 1000 * 86400000;
  let wk = { 技能: {
    '弱·法':   { 名: '弱·法',   方法: 'm', 触发: ['w'], 来源: '受教', 验证: false, 例: [], count: 3, first_ts: 1, last_ts: NOWs - 40 * 86400000, 强度: 3 },
    '强·法':   { 名: '强·法',   方法: '验证方法', 触发: ['s'], 来源: '习得', 验证: true,  例: [], count: 3, first_ts: 1, last_ts: NOWs - 40 * 86400000, 强度: 4 },
    '活·法':   { 名: '活·法',   方法: 'm', 触发: ['a'], 来源: '受教', 验证: false, 例: [], count: 6, first_ts: 1, last_ts: NOWs - 40 * 86400000, 强度: 8 },
    '新近·法': { 名: '新近·法', 方法: 'm', 触发: ['r'], 来源: '受教', 验证: false, 例: [], count: 3, first_ts: 1, last_ts: NOWs - 5 * 86400000,  强度: 3 },
  }, 候选: {}, 总数: 4 };
  wk = S.skillUpsert(wk, { 名: '触发·法', 方法: 'x', 触发: ['t'], 来源: '习得', 验证: true, ts: NOWs });   // 任一次写入即触发弱技能扫描
  ok('弱技能自动删除：未验证+低强度+过期 被清', !wk.技能['弱·法']);
  ok('验证技能永不自动删（即便过期）', !!wk.技能['强·法']);
  ok('常用未验证技能（强度高）不被误删', !!wk.技能['活·法']);
...
// ── 显式事实记忆:主人明说的立刻抓成长驻事实(不等溢出) ──
{
  ...
  ok('事实·「以后…」闲聊不误存', SF.extractFacts('以后再说吧').every(f => !/^以后/.test(f)) && SF.extractFacts('以后有空聊').every(f => !/^以后/.test(f)));
  ok('事实·「以后要…」承诺仍收', SF.extractFacts('以后要每天写日报').some(f => /^以后.*日报/.test(f)));

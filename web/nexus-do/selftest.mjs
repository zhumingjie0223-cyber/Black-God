...
// ── 亲友令牌边界（本次代码审查修复的回归测试）──
// 1) 受限提示词绝不含主人能力清单与 ⟨召唤⟩ 调度语法（提示词泄露 = 越权幻觉源头）
{
  const full = S.STABLE_SYSTEM_PREFIX();          // 主人上下文：默认含能力
  const bare = S.STABLE_SYSTEM_PREFIX(false);     // 受限上下文：必须不含
  ok('主人前缀含召唤语法', full.includes('召唤') && full.includes('能力'));
  ok('受限前缀无召唤语法', !bare.includes('召唤:') && !bare.includes('⟨召唤'));
  ok('受限前缀无能力清单', !bare.includes('此刻真实拥有的能力'));
  const trusted = S.TRUSTED_SYSTEM_PREFIX({ 心绪: 0.5 }, '老妈');
  ok('亲友前缀无召唤语法·带边界声明', !trusted.includes('⟨召唤') && trusted.includes('不是主人本人'));
}
// 2) 亲友回复清洗：即便模型吐了召唤标记也不外露
{
  const { cleanReply } = S.parseSummons('好的我记下了⟨召唤:tg｜给主人发消息⟩就这样');
  ok('召唤标记被剥离', !cleanReply.includes('召唤') && cleanReply.includes('记下了'));
}
// 3) 限流池隔离：pub 池刷爆不影响 trust 池
{
  const R = Object.create(ShenshuCore.prototype);
  for (let i = 0; i < 130; i++) R._pubRateOk('attacker' + i, 'pub');
  ok('pub 池被刷爆后自身拒绝', R._pubRateOk('one-more', 'pub') === false);
  ok('trust 池不受 pub 池连带', R._pubRateOk('trust:abc', 'trust') === true);
}
// 4) 压缩保尾：最新资料原文必须在压缩产物里（摘要失败退硬截断时同样保尾）
{
  const long = 'x'.repeat(9000) + '关键答案在结尾';
  const fallback = long.slice(-6000);
  ok('硬截断兜底保留尾部', fallback.includes('关键答案在结尾'));
}

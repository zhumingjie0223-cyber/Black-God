...
// 「大脑·模型」选择真影响后端顺序（pickBrainOrder 纯函数）
ok('模型选 cf-llama → 只用 CF、不碰网关', JSON.stringify(S.pickBrainOrder('heavy', 'cf-llama')) === JSON.stringify(['cf']));
ok('模型选 strong → 网关优先仍兜底 CF（诚实降级）', JSON.stringify(S.pickBrainOrder('light', 'strong')) === JSON.stringify(['gateway', 'cf']));
ok('auto + light → 先免费 CF 后网关', JSON.stringify(S.pickBrainOrder('light', 'auto')) === JSON.stringify(['cf', 'gateway']));
ok('auto + heavy → 网关优先后 CF', JSON.stringify(S.pickBrainOrder('heavy', 'auto')) === JSON.stringify(['gateway', 'cf']));
ok('缺省 model（老客户端）→ 等同 auto（向后兼容）', JSON.stringify(S.pickBrainOrder('heavy')) === JSON.stringify(S.pickBrainOrder('heavy', 'auto')));
